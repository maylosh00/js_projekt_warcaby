    ...
    def _drawBoard(self, window):
        ...
        for i in range(ROWS):
            ...
            window.blit(text, (
                ...
                y + BORDER_SIZE / 2 - text.get_height() / 2))
        ...
        for row in range(start, end, direction):
            ...
            if currentSquare == 0:
                ...
                if skippedPawns and not previousSquare:
                    break
                # jeżeli nic a przez poprzednie pole już przeskoczyliśmy, wychodzimy z pętli
                # (ten warunek zabezpiecza przed "wracaniem się" algorytmu w przypadku możliwości wielokrotnego bicia
                # do tyłu
                elif previousSquare and previousSquare[0] in skippedPawns:
                    break
                ...
                if previousSquare:
                    ...
                    moves.update(
                        self._checkDiagonal(left, row + direction, newEnd, direction, color, column - 1, isKing,
                                            skippedPawns=moves[(row, column)]))
                    moves.update(
                        self._checkDiagonal(right, row + direction, newEnd, direction, color, column + 1, isKing,
                                            skippedPawns=moves[(row, column)]))
            ...
            # jeżeli pole jest pionkiem tego samego koloru, nie mogę się tu ruszyć
                ...
                # jeżeli nie jest, ustawiam obecne pole jako poprzednie i przechodzę do następnej iteracji
                    ...
                    # warunek nie pozwalający damkom przeskoczyć dwóch pionków stojących obok siebie
                    if previousSquare:
                        break
            ...
            if ALLOW_SKIPPING_BACKWARDS:
                ...
                movesToCheck.update(
                    self._checkDiagonal(left, row - direction, endRowBackwards, -direction, pawn.getColor(), leftColumn,
                                        isinstance(pawn, KingPawn)))
                movesToCheck.update(
                    self._checkDiagonal(right, row - direction, endRowBackwards, -direction, pawn.getColor(),
                                        rightColumn,
                                        isinstance(pawn, KingPawn)))
